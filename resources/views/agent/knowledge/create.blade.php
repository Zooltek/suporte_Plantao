...
@section('title', 'EasyWiki — Novo Artigo')
...
<div class="max-w-3xl mx-auto space-y-5">
    ...
    <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
        ...
        <span class="text-gray-600">Novo Artigo</span>
    </nav>
    ...
    @if($ticket)
        <div class="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
            ...
            <div>
                <p class="font-bold text-amber-800">Criando a partir do Chamado #{{ $ticket->id }}</p>
                <p class="text-amber-700 text-xs">{{ $ticket->subject }}</p>
            </div>
            <a href="{{ route('agent.ticket.show', $ticket->id) }}"
               class="ml-auto text-xs font-bold text-amber-600 hover:text-amber-800 transition-colors">
                Ver chamado →
            </a>
        </div>
    @endif

    <form method="POST" action="{{ route('agent.knowledge.store') }}"
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50">
            <h1 class="text-lg font-extrabold text-gray-900">Incluir na EasyWiki</h1>
            <p class="text-xs text-gray-500 mt-0.5">Documente o problema e a solução para consultas futuras</p>
        </div>
        ...
        <div class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title"
                       value="{{ old('title', $ticket ? $ticket->subject : '') }}"
                ...
                <select name="category_id"
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->category_id }}"
                                @selected(old('category_id', $ticket?->category_id) == $cat->category_id)>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            {{-- Problema --}}
            <div x-data="expandableText({{ json_encode(old('problem', $ticket?->trouble ?? '')) }}, 'Descrição do Problema')">
            ...
            {{-- Solução --}}
            <div x-data="expandableText({{ json_encode(old('solution', $ticket?->solution ?? '')) }}, 'Solução')">
                <div class="flex items-center justify-between mb-1.5">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-widest">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="visibility"
                            class="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl
                                   outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                        <option value="internal" @selected(old('visibility', 'internal') === 'internal')>
                            Interno (somente equipe)
                        </option>
                        <option value="public" @selected(old('visibility') === 'public')>
