...
<div class="py-8">
    <div class="mb-6">
        ...
        <p class="text-gray-500">Editar categoria existente</p>
    </div>

    <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <form x-data="ticketitCategoryEditForm({ id: {{ $category->id }}, name: '{{ addslashes($category->name) }}', color: '{{ $category->color ?? '#3B82F6' }}' })" @submit.prevent="submit()" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
