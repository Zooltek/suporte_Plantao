...
<div class="py-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Criar Configuração</h1>
        <p class="text-gray-500">Adicionar nova chave de configuração</p>
    </div>

    <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <form x-data="configurationForm()" @submit.prevent="submit()" class="space-y-4">
            @csrf
            <div>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" :disabled="submitting" class="px-4 py-2 bg-blue-600 text-white rounded">
                    <span x-show="!submitting">Criar Configuração</span>
        ...
        return {
            ...
            async submit(){
                ...
                try{
                    ...
                    if(res.ok){ AppToast.success({ message: 'Configuração criada com sucesso' }); window.setTimeout(()=>{ window.location.href='{{ route('tickets.admin.configuration.index') }}'; },800); }
                    else { const j=await res.json().catch(()=>({message:'Erro'})); AppToast.error({ message: j.message||'Erro ao criar configuração' }); }
