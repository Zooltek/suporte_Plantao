...
@section('page-title', 'Suporte - Solicitações por Módulo')
@section('title', 'Suporte - Solicitações por Módulo')
...
<style>
    ...
    html.ocean .print-module-dialog-title {
        color: #f8fafc !important;
    }

    html.ocean .print-module-dialog-subtext {
        color: #94a3b8 !important;
    }

    html.ocean .print-module-item-name {
        color: #e2e8f0 !important;
    }

    html.ocean .print-module-item-meta {
        color: #94a3b8 !important;
    }
    ...
    html.ocean .report-card .report-toggle-icon {
        color: #94a3b8 !important;
    }
    ...
    html.ocean .report-card tbody {
        border-color: #334155 !important;
    }

    html.ocean .report-card tbody tr {
        border-color: #334155 !important;
    }

    html.ocean .report-card tbody tr:hover {
        background-color: #1e293b !important;
    }

    html.ocean .report-card td {
        color: #e2e8f0 !important;
    }
    ...
    html.ocean .report-card .status-badge {
...
@php
    $totalRequests = collect($data)->sum(fn($row) => count($row['tasks'] ?? []));
    $totalModules = collect($data)->filter(fn($row) => count($row['tasks'] ?? []) > 0)->count();
    $printableModules = collect($data)
        ->filter(fn($row) => count($row['tasks'] ?? []) > 0)
        ->values()
        ->map(fn($row, $index) => [
            'id' => isset($row['module']) ? (string) $row['module']->id : "module-{$index}",
            'name' => isset($row['module']) ? $row['module']->name : 'Módulo não definido',
            'count' => count($row['tasks'] ?? []),
        ])
        ->values()
        ->all();
...
<div x-data="taskReportPrint(@js($printableModules))" x-init="init()" @keydown.escape.window="closePrintDialog" class="report-shell mx-auto space-y-6">
    <header class="bg-white border border-slate-200 rounded-2xl shadow-sm px-5 py-5 sm:px-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-semibold text-slate-900 tracking-tight">Solicitações por Módulo</h1>
                <p class="text-sm text-slate-500 mt-1">
                    Visão consolidada das tarefas ativas por módulo.
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 no-print">
                ...
                <button @click="openPrintDialog" type="button" class="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-sky-700">
        ...
        <div class="absolute inset-x-0 bottom-0 max-h-[85vh] overflow-hidden rounded-t-2xl shadow-2xl sm:inset-x-auto sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[38rem] sm:max-w-[96vw] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl">
            <div class="print-module-dialog flex h-full max-h-[85vh] flex-col">
                <div class="border-b border-slate-200 px-5 py-4">
                    <h3 class="print-module-dialog-title text-base font-semibold text-slate-900">Selecionar módulos para impressão</h3>
                    <p class="print-module-dialog-subtext mt-1 text-sm text-slate-500">
                        Escolha um, vários ou todos os módulos.
                    </p>
                </div>

                <div class="flex flex-wrap items-center gap-2 border-b border-slate-200 px-5 py-3">
                    <button @click="selectAllModules" type="button" class="print-dialog-secondary rounded-md px-3 py-1.5 text-xs font-semibold transition-colors">
                        Selecionar todos
                    </button>
                    <button @click="clearSelection" type="button" class="print-dialog-secondary rounded-md px-3 py-1.5 text-xs font-semibold transition-colors">
                        Limpar seleção
                    </button>
                ...
                <div class="flex flex-col gap-2 border-t border-slate-200 px-5 py-4 sm:flex-row sm:justify-end">
                    <button @click="closePrintDialog" type="button" class="print-dialog-secondary w-full rounded-lg px-4 py-2.5 text-sm font-semibold transition-colors sm:w-auto">
                        Cancelar
                    </button>
                    <button @click="printSelectedModules" type="button" class="w-full rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto" :disabled="selectedModuleIds.length === 0">
    ...
    <div class="space-y-5">
        @foreach($data as $row)
            @php
                $moduleTasks = $row['tasks'] ?? [];
                $moduleId = isset($row['module']) ? (string) $row['module']->id : "module-{$loop->index}";
            @endphp

            @if(count($moduleTasks) > 0)
                <section x-data="{ open: true }" data-report-module-id="{{ $moduleId }}" class="report-card overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <h2 class="flex items-center gap-2 text-base font-semibold tracking-wide text-slate-800 sm:text-lg">
                                <svg class="h-5 w-5 text-sky-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 4a2 2 0 1 1 4 0v1a1 1 0 0 0 1 1h3a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1a2 2 0 1 0 0 4h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-3a1 1 0 0 1-1-1v-1a2 2 0 1 0-4 0v1a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1v-3a1 1 0 0 0-1-1H4a2 2 0 1 1 0-4h1a1 1 0 0 0 1-1V7a1 1 0 0 1 1-1h3a1 1 0 0 0 1-1V4z" />
                                </svg>
                                {{ isset($row['module']) ? $row['module']->name : 'Módulo não definido' }}
                            </h2>
                            <div class="flex items-center gap-2">
                                <span class="module-count rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-600">
                                    {{ count($moduleTasks) }} itens
                                </span>
                                <svg class="report-toggle-icon h-4 w-4 text-slate-500 transition-transform duration-200 no-print" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div x-show="open" x-collapse x-cloak class="report-card__body p-0 sm:p-2">
                            ...
                            <table class="min-w-full border-collapse text-left text-[13px] sm:text-[14px]">
                                <thead>
                                    <tr class="border-b border-slate-200 bg-slate-50 text-slate-600">
                                        <th class="w-[110px] px-3 py-2 font-semibold">Criação</th>
                                        <th class="w-[260px] px-3 py-2 font-semibold">Cliente</th>
                                        ...
                                        <tr class="transition-colors hover:bg-sky-50/50">
                                            ...
                                            <td class="px-3 py-2">
                                                @if(isset($task->customer))
                                                    <span class="font-medium text-slate-800">{{ $task->customer->trade_name }}</span>
                                                @else
                                                    <span class="text-sm italic text-slate-400">&laquo; Cliente não definido &raquo;</span>
                                                @endif
                                            </td>

                                            <td class="px-3 py-2 text-slate-700">{{ $task->title }}</td>
