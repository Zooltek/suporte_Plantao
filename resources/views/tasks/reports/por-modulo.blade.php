...
@section('page-title', 'Suporte - Label por Módulo')
@section('title', 'Suporte - Label por Módulo')
...
@php
    ...
    $totalTasks    = collect($data)->sum(fn($p) =>
        collect($p['labels'])->sum(fn($l) => count($l['tasks']))
    );

    /* Mapeia projetos para o dialog de impressão */
    $printableGroups = collect($data)
        ->filter(fn($p) => collect($p['labels'])->some(fn($l) => count($l['tasks']) > 0))
        ->values()
        ->map(fn($p, $i) => [
            'id'    => "project-{$i}",
            'name'  => $p['project_name'],
            'count' => collect($p['labels'])->sum(fn($l) => count($l['tasks'])),
        ])
        ->values()
        ->all();
    ...
    <header class="bg-white border border-slate-200 rounded-2xl shadow-sm px-5 py-5 sm:px-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-semibold text-slate-900 tracking-tight">Label por Módulo</h1>
                <p class="text-sm text-slate-500 mt-1">Permite identificar o volume de solicitações por área funcional do sistema, facilitando a priorização de módulos com maior demanda.</p>
            </div>
    ...
    <div class="space-y-5">
        @foreach($data as $projectIndex => $project)
            @php
                ...
                $projectTotal = collect($project['labels'])->sum(fn($l) => count($l['tasks']));
            @endphp
            ...
            @if($projectTotal > 0)
                <section
                    ...
                    <div x-show="open" x-collapse x-cloak class="report-card__body divide-y divide-slate-100">
                        ...
                        @foreach($project['labels'] as $row)
                            @if(count($row['tasks']) > 0)
                                <div class="p-4 sm:p-5 space-y-3">

                                    {{-- Sub-cabeçalho da etiqueta --}}
                                    <h3 class="label-heading flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-slate-700 border-l-4 border-orange-400 pl-3">
                                        ...
                                        <table class="min-w-full border-collapse text-left text-[13px] sm:text-[14px]">
                                            <thead>
                                                <tr class="border-b border-slate-200 bg-slate-50 text-slate-600">
                                                    ...
                                                    <th class="w-[200px] px-3 py-2 font-semibold">Cliente</th>
                                            ...
                                            <tbody class="divide-y divide-slate-100">
                                                @foreach($row['tasks'] as $task)
                                                    <tr class="transition-colors hover:bg-sky-50/50">
                                                        ...
                                                        <td class="px-3 py-2">
                                                            @if($task->customer)
                                                                <span class="font-medium">{{ $task->customer->trade_name ?? $task->customer->name }}</span>
                                                            @else
                                                                <span class="text-sm italic text-slate-400">&laquo; Sem cliente &raquo;</span>
                                                            @endif
                                                        </td>
                                                        <td class="px-3 py-2 text-slate-800">{{ $task->title }}</td>
